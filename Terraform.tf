...
cd /var/www/html
wget https://raw.githubusercontent.com/gunashrie/terraform/master/moh.html 
EOF
...
resource "aws_lb" "terraform-alb" {
  ...
  enable_http2 = false
  security_groups = [aws_security_group.public_securitygroup.id]
  subnets         = [
